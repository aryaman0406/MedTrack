...
# Try to install system dependencies for OCR and PDF generation (optional, non-fatal)
if command -v apt-get &> /dev/null; then
    ...
    (apt-get update && apt-get install -y \
        ...
        libgdk-pixbuf2.0-0 \
        libffi-dev \
        shared-mime-info) || echo "Some system dependencies could not be installed (non-fatal, continuing build)"
fi
